...
use rabe::utils::policy::pest::{PolicyLanguage, parse, PolicyType};
...
impl ToString for Ac17PublicKey {
    fn to_string(&self) -> String {
        let g_str = format!("{:?}", self._g);
        let h_a_str = format!("{:?}", self._h_a);
        let e_gh_ka_str = format!("{:?}", self._e_gh_ka);
        format!("Ac17PublicKey {{\n  _g: {},\n  _h_a: {},\n  _e_gh_ka: {}\n}}", g_str, h_a_str, e_gh_ka_str)
    }
}

impl FromStr for Ac17PublicKey {
    type Err = String;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        // 这里需要编写解析字符串的逻辑
        // 从字符串中提取字段值并构建 Ac17PublicKey 实例
        // 这是一个示例，您可能需要根据实际需要编写更复杂的逻辑
        // 此示例假定字符串格式与上述 to_string 方法生成的格式匹配
        if let Some(g_str) = s.find("_g: ") {
            let g_end = s.find(",").ok_or("Invalid format")?;
            let _g = G1::from_str(&s[(g_str + 4)..g_end]).map_err(|e| e.to_string())?;

            if let Some(h_a_str) = s.find("_h_a: ") {
                let h_a_end = s.find(",").ok_or("Invalid format")?;
                let _h_a = Vec::<G2>::from_str(&s[(h_a_str + 6)..h_a_end]).map_err(|e| e.to_string())?;

                if let Some(e_gh_ka_str) = s.find("_e_gh_ka: ") {
                    let e_gh_ka_end = s.find("}}").ok_or("Invalid format")?;
                    let _e_gh_ka = Vec::<Gt>::from_str(&s[(e_gh_ka_str + 11)..e_gh_ka_end]).map_err(|e| e.to_string())?;

                    Ok(Ac17PublicKey { _g, _h_a, _e_gh_ka })
                } else {
