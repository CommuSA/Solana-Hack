...
pub fn process_instruction(
    ...
    match instruction {
        IntroInstruction::InitUserInput { name, message } => {
            add_stuff_intro(program_id, accounts, name, message)
        }
        IntroInstruction::UpdateStuffIntro { name, message } => {
            update_stuff_intro(program_id, accounts, name, message)
        }
        IntroInstruction::AddReply { reply } => add_reply(program_id, accounts, reply),
    }
}

pub fn add_stuff_intro(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    name: String,
    message: String,
) -> ProgramResult {
    msg!("Adding stuff intro...");
    msg!("Name: {}", name);
    msg!("Message: {}", message);
    ...
    if pda != *user_account.key {
        ...
        return Err(StuffIntroError::InvalidPDA.into());
    }
    let stuffinfo_discriminator = "stuffinfo";
    let total_len: usize =
        (4 + stuffinfo_discriminator.len()) + 1 + (4 + name.len()) + (4 + message.len());
    if total_len > 1000 {
    ...
    let mut account_data =
        try_from_slice_unchecked::<StuffInfo>(&user_account.data.borrow()).unwrap();
    msg!("borrowed account data");

    msg!("checking if stuffinfo account is already initialized");
...
pub fn update_stuff_intro(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    name: String,
    message: String,
) -> ProgramResult {
    msg!("Updating stuff intro...");
    msg!("Name: {}", name);
    msg!("Message: {}", message);
    let account_info_iter = &mut accounts.iter();
    ...
    if !account_data.is_initialized() {
        ...
        return Err(StuffIntroError::UninitializedAccount.into());
    }
...
pub fn add_reply(program_id: &Pubkey, accounts: &[AccountInfo], reply: String) -> ProgramResult {
    ...
    reply_data.stuffinfo = *user_account.key;
